...
class Stage:
    ...
    def getConfigElements(self)->list[StageConfigElement]:
        return [
            StageConfigElement(text="Stage Name", data_type=str, data=self.name),
    ...
    def setConfigElements(self, config_elements:list[StageConfigElement]):
        for elem in config_elements:
            if elem.text == "Stage Name":
                if not isinstance(elem.data, str):
                    raise TypeError("Stage Name must be a string")
                self.name = elem.data
            elif elem.text == "Visible":
...
class StepPipeline(Pipeline):
        ...
        def run(self, input_data):
            ...
            for i in range(start, len(self.stages)):
                stage = self.stages[i]
                if(hasattr(stage, "setStage") and callable(stage.setStage)):
                    # reset stage state to WAITING
                    stage.setStage(StageState.WAITING)
