...
class CnnChairTransformer(AbcImageTransformer):
    ...
    def process(self, data: PipeDataClass) -> PipeDataClass:
        ...
        data.remove_optional_layer(self.__class__.__name__)
        if(self._stage_visible==False): 
            return data
        frame = cv2.cvtColor(np.array(data.base_image), cv2.COLOR_RGBA2BGRA)
        ...
        # 2.5) Ensure both images have an alpha channel (BGRA)
        if frame.shape[2] == 3:
        ...
        warped_rgb = cv2.cvtColor(warped, cv2.COLOR_BGRA2RGBA)
        poster_layer = Image.fromarray(warped_rgb)

        # 7) Attach as new layer in the PipeDataClass
        data.add_optional_layer(self.__class__.__name__,poster_layer)
