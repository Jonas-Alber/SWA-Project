...
class CnnChairTransformer(AbcImageTransformer):
    def __init__(self):
        ...
        if 'tvmonitor' not in self.coco_names and 'tv' in self.coco_names:
            self.target_object_names[self.target_object_names.index('tvmonitor')] = 'tv'
        
        self.detection_targets = {name: (True if name == "chair" else False) for name in self.target_object_names}

        # Validate that all target names are in COCO names
        for name in self.target_object_names:
            if name not in self.coco_names:
                print(f"Warning: Target class '{name}' not found in COCO names. Detection for this class might not work.")
            ...
            for i in range(len(pred_labels)):
                ...
                if score > self.confidence_threshold:
                    ...
                    if class_name in self.detection_targets and self.detection_targets[class_name]:
                        ...
                        x1, y1, x2, y2 = box
                        draw.rectangle([(x1, y1), (x2, y2)], outline="red", width=3) # You could vary color by class_name
